# Weibo Text Archiver

把个人微博历史整理成属于自己的本地 Markdown 归档，便于长期阅读、人工核验与 AI 分析。

归档过程在本机完成。程序不内置 LLM，也不会把归档自动上传到云端；是否把导出文件交给 ChatGPT、Claude、Gemini、DeepSeek、Qwen、豆包或其他第三方服务，始终由用户自行决定。

## 它能做什么

- 一次读取可生成完整归档、AI 分析版和自定义导出中的任意组合。
- 支持全量快照、最近 N 条、起始日期范围和最近 20 条的测试导出。
- 通过明确支持的路径取得长微博正文。
- 对当前无法验证的历史内容做显式标记，不把列表预览冒充完整正文。
- 将归档完整性与分页是否结束分开报告。
- 输出本地 Markdown，不使用浏览器自动化，也不在运行时下载依赖。
- 提供可选、独立的“图片备份…”工具，将本次微博接口明确返回且可下载的图片保存在本机。

## 快速开始

1. 解压 Windows ZIP，运行 `WeiboTextArchiver.exe`。
2. 扫码登录微博，输入目标账号的数字 UID。
3. 如果不确定如何选择，保留默认勾选的“完整归档”和“AI 分析版”。
4. 可以先用“测试导出”验证流程；它只读取最近 20 条，不代表完整归档。
5. 正式导出后，文件默认保存在程序旁的 `Archives` 文件夹；若该位置不可写，则使用 `Documents\WeiboTextArchiver\Archives`。
6. 用完整归档进行长期保存、人工阅读和未来核验。
7. 需要模型分析时，再由你选择是否把 AI 分析版上传到第三方 AI 服务。

## 图片备份

在主窗口扫码登录后，点击“图片备份…”打开独立窗口。图片备份不与文本导出绑定，图片失败或枚举缺口不会改变文本归档的成功与完整性状态。

- 默认“测试备份 20 条”，也支持最近 N 条、从指定日期起和全量备份。置顶微博可作为额外记录保留，不消耗普通时间线目标数。
- 保存本次微博接口明确返回且可下载的图片，包括普通图片、GIF 和 Live Photo 的静态部分；顶层微博与转发原文分别记录归属。
- 视频、Live Photo 动态部分、卡片封面、头像和文章媒体不在范围内；不承诺全部历史图片或上传原始质量。
- 在选择的基础目录下按账号保存图片和本地进度记录。再次运行同一备份时，已有图片会先校验，确认完整后不会重复下载。
- 微博声明的媒体数量可能超过返回的图片槽位数量；结果会分别报告数量和枚举缺口，不把未返回的图片当作已保存。
- 全量图片备份可能占用较多磁盘空间并耗时较长，建议先用默认测试范围。取消或部分失败时，已成功保存的文件会保留。

## 应该选择哪种输出？

### 完整归档

面向人工阅读和长期保存的归档版本，适合：

- 长期保留个人微博历史；
- 按时间阅读和人工核验；
- 为未来重新整理或迁移保留可靠参考。

它不是微博服务器数据的逐字节镜像，也不会把当前无法访问或无法验证的内容伪装成完整正文。
完整归档不会按可见范围排除记录；它保存当前登录会话在本次抓取中实际返回的全部规范化记录，并明确标注抓取时观察到的可见范围。

### AI 分析版

针对语言模型分析优化的确定性结构化表示：

- 不会在导出过程中调用任何 LLM；
- 不是 AI 自动生成的摘要；
- 已验证且未重复的正文不会被有意摘要或删减；
- 明确区分目标账号的顶层文字与嵌套转发原文；
- 明确标注来源、归属、不确定性、缺失与预览边界；
- 对重复转发正文使用显式引用，减少不必要的重复。

AI 分析版默认只纳入可确认是公开范围的顶层微博。粉丝可见、好友圈和仅自己可见内容需要用户明确选择后才会加入；可见范围未知的记录不会被自动当作公开内容。可见范围表示归档抓取时来源响应中观察到的元数据，不证明最初设置或此后未发生变化。

不同模型可能给出不同分析结果。把文件上传到第三方服务意味着由用户主动向该服务披露文件内容，请根据自己的隐私需求决定是否使用。

### 自定义导出

在本机对本次已读取记录生成确定性工作集。可以选择部分元数据，并按原创/转发、关键词、可选日期范围和可选可见范围筛选；不会为了筛选再次联网。

关键词对 ASCII 不区分大小写并使用子串匹配。多个关键词使用 OR；同一可见范围维度内使用 OR；类型、关键词、日期和可见范围维度之间使用 AND。当前模型没有可靠的结构化话题字段，如需筛选话题，可直接把 `#话题#` 作为普通关键词。

## 可以用 AI 分析版问什么？

- 我的兴趣在这些年里发生了哪些变化？请引用具体微博作为证据。
- 哪些游戏、作品、人物或主题被我反复提到，却可能已经忘记？
- 我的表达方式或写作风格是否随时间发生变化？
- 请按时间整理我明确提到的重要事件，不要补充归档中不存在的信息。
- 我明确说去过哪些地方？请区分“去过”“只在正文中提到”和微博显示的发布位置。
- 哪些观点随时间发生变化？请列出相互支持或矛盾的原文证据。
- 哪些旧微博后来被我再次转发？请区分顶层评论与转发原文。
- 哪些结论证据充分，哪些只是可能的解释？请分别说明依据和不确定性。

`I`、`V`、`A` 等媒体标记只描述归档识别到的媒体事实，图片、视频或文章内容本身并未导出。不要要求模型凭这些标记猜测未看到的媒体内容。

## 重要边界

- 时间线中可见的预览不等于已经验证的完整正文。
- 只有满足严格证据条件时，当前不可访问内容才会被明确标为不完整。
- 登录、验证、限流、网络、未知结构或大范围访问异常会使任务停止，而不是产生看起来完整的归档。
- AI 分析仍受导出证据限制，结构化格式不能保证模型不产生错误推断。
- 微博可能调整未公开接口、返回结构或访问策略。
- 完整归档不等于公开主页，AI 分析版也不等于完整归档。

## Windows

Windows 包面向 Windows 10/11 x64，不需要预先安装 Python。

从最新 GitHub Release 下载 `WeiboTextArchiver_Windows.zip`，解压后运行：

```text
WeiboTextArchiver.exe
```

也可以在命令提示符中下载最新正式 Release：

```cmd
curl.exe -fL -o WeiboTextArchiver_Windows.zip https://github.com/Berunkasuteru/weibo-text-archiver/releases/latest/download/WeiboTextArchiver_Windows.zip
```

## 从源码运行

```text
python -m weibo_archive.app
```

## 隐私与安全

- 程序每次启动会在后台向公开的 GitHub latest-release 接口检查一次新版本；不会发送微博 UID、登录凭据、归档内容或导出设置。检查失败会静默忽略，不影响正常使用。
- 登录凭据只保存在本机，并用于直接请求微博；项目不运营凭据上传服务。Windows 会使用当前用户的 DPAPI 保护已保存凭据；非 Windows 源码模式保留本地明文兼容方式。
- 不使用浏览器自动化，不关闭 TLS 证书验证。
- Markdown 与规范化归档缓存都保存在本机。
- 图片文件和图片进度记录同样保存在本机；实际图片 CDN 请求不携带微博 Cookie 或保存的登录凭据，图片进度记录不保存远程图片 URL。
- DPAPI 提供静态存储保护，但不能防御已在同一 Windows 用户身份下运行的恶意软件。诊断信息会经过脱敏，仍不应向他人发送 `credential.dat` 或旧版 `cookie.txt`。
- “清除登录信息”只删除登录/会话材料，不会删除缓存或已经导出的 Markdown。
- 工具只归档当前登录会话实际返回的记录，不推断或枚举当前会话无法访问的微博；可见范围筛选只在本地 Archive 上执行。

## 开发与构建

```text
python tools/build/environment_check.py
python tests/run_tests.py
python -B -m unittest discover -s tests -p test_image_core.py
python -B -m unittest discover -s tests -p test_image_ui.py
```

`python tests/run_tests.py` 运行 70 项文本/应用回归测试组；另外两条命令分别运行 96 项图片核心测试和 41 项图片 UI 测试。发布前须运行全部三套，共 207 项；UI 测试需要可用的 Tk 桌面环境。测试不需要连接微博。

在 Windows 上运行 `tools\windows\BUILD_WINDOWS.bat` 可使用隔离的 `.venv-build` 环境生成 PyInstaller `onedir` / `windowed` ZIP，并在 `release/` 下写入 `SHA256.txt`。

## License

Weibo Text Archiver 使用 MIT License。第三方组件仍受各自许可证约束，详见 `THIRD_PARTY_NOTICES.txt` 与 `THIRD_PARTY_LICENSES/`。
